...
You can use Identity Authentication for authentication in OpenID Connect \(OIDC\) protected applications.

OpenID Connect \(OIDC\) is a simple identity layer on top of the OAuth 2.0 protocol. Clients can verify the identity of the end-user based on the authentication performed by an authorization server, as well as to obtain basic profile information about the end-user in an interoperable and REST-like manner. For more information about the OpenID Connect \(OIDC\) specification, see [OpenID Connect Core 1.0](https://openid.net/specs/openid-connect-core-1_0.html).

The OpenID Connect \(OIDC\) implementation of Identity Authentication supports all flow from OpenID Connect \(OIDC\) Core 1.0.
