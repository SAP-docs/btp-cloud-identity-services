...
When the *Multi-Factor Authentication* option in the administration console is switched on, the users can choose to always provide second factor when authenticating. User can face two scenarios, when enabling the *Multi-Factor Authentication* option on their profile pages:

-   An application doesn't require a second factor for authentication, or the application has configured risk-based authentication rules, but the user doesn't fall into those rules that require second factor for authentication. In this scenario, the users are required to provide a second factor.

    If users chose the *Trust this browser* checkbox, the second factor is required only with the first logon. The validity of *Trust this browser* is 3 months.

-   An application requires a second factor for authentication, or the application has configured risk-based authentication rules, and the users fall into those rules that require second factor for authentication. In this scenario, users are always required to provide a second factor. The *Trust this browser* checkbox does not appear.
