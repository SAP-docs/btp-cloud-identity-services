...
# Update SAP S/4HANA Cloud

Update the SAP Integrated Business Planning connector to a new version.

The SAP S/4HANA Cloud connector supports two versions, corresponding to the APIs used for integration with Identity Provisioning:
...
Updating the connector to version 3 depends on your provisioning flow, specifically whether the SAP S/4HANA Cloud system is configured as a source or a target system in that flow.
...
> ### Note:  
> If you have only an SAP S/4HANA Cloud source system, or if it is connected in a flow with a target system such as Identity Authentication version 2 or Local Identity Directory, always start the update from the source system.

Proceed as follows:

1.  Open your SAP S/4HANA Cloud source system and verify that it is configured for version 1. The system is on version 1 if the `ibp.api.version` property is either not set or set to `1`.

2.  Select the *Properties* tab and set the following:

    -   *s4hana.cloud.api.version*=`3`

    -   *ips.application.id*=*<The ID of the application that is created under Applications & Resources for the corresponding SAP S/4HANA Cloud provisioning system\>*.
...
You have an SAP S/4HANA Cloud version 1 target system, that is connected with source systems such as Identity Authentication version 2 or Local Identity Directory.
...
1.  Open your source system, respectively Identity Authentication version 2 or Local Identity Directory and ensure that it has the following mapping for `externalName` attribute.
    ...
    > ### Note:  
    > Groups in Identity Authentication version 2 or Local Identity Directory source systems must have the `externalName` attribute to be provisioned successfully to an SAP S/4HANA Cloud version 2 target system.

2.  Open the SAP S/4HANA Cloud version 1 target system and set the following on the *Properties* tab:
    ...
    -   Remove unnecessary properties that are valid for SAP S/4HANA Cloud version 1.
...
1.  Open your SAP S/4HANA Cloud proxy system and verify that it is configured for version 1.
