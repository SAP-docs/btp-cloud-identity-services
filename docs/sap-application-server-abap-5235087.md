...
7.  \(Optional\) Configure the transformations.
    ...
    > ### Note:  
    > The value of the **entityIdSourceSystem** attribute stores the unique ID of the entity. Do not delete or change this statement! You can exchange the default attribute **USERNAME** \(which is used as a source\) with another one, but make sure it is unique.
